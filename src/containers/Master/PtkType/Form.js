  ...
  const onFinish = (values) => {
    form
      ...
      .then(async () => {
        setLoading(true)
        if (mode === 'Tambah') {
          await storePtkType(values).then((res) => {
            callNotify(notify, res, mode, setLoading, getData, setMode, onReset)
          })
        } else {
          await updatePtkType(itemEdit.id, values).then((res) => {
            callNotify(notify, res, mode, setLoading, getData, setMode, onReset)
          })
        }
      })
