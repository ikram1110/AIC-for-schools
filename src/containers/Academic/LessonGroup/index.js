...
import { deleteLessonGroup, getAllLessonGroup } from 'src/services/lessongroup'
import makeColumns from 'src/utils/column'
import fields from './fields'
import { getAllUnit } from 'src/services/unit'
import LessonGroupForm from './Form'
...
const LessonGroup = () => {
  ...
  const getData = async () => {
    setLoading(true)
    const response = await getAllLessonGroup()
    setSource(response)
  ...
  return (
    <>
      ...
      {mode === 'Data' ? (
        <Card
          title="Data Kelompok Mata Pelajaran"
          extra={
