...
import { storeLessonGroup, updateLessonGroup } from 'src/services/lessongroup'
import DefaultForm from 'src/components/DefaultForm'
import fields from './fields'
import callNotify from 'src/utils/notify'

const LessonGroupForm = (props) => {
  ...
  const onFinish = (values) => {
    form
      ...
      .then(async () => {
        setLoading(true)
        if (mode === 'Tambah') {
          await storeLessonGroup(values).then((res) => {
            callNotify(notify, res, mode, setLoading, getData, setMode, onReset)
          })
        } else {
          await updateLessonGroup(itemEdit.id, values).then((res) => {
            callNotify(notify, res, mode, setLoading, getData, setMode, onReset)
          })
        }
      })
  ...
  return (
    <Card
      title={mode + ' Data Kelompok Mata Pelajaran'}
      extra={
